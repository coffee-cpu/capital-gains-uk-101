...
test.describe('CSV Import', () => {
  ...
  test('should import EquatePlus CSV successfully', async ({ page }) => {
    await page.goto('/')
    ...
    // Upload the EquatePlus test CSV file
    const filePath = path.join(__dirname, 'fixtures', 'equateplus-sample.csv')
    await fileInput.setInputFiles(filePath)

    // Wait for success message
    await expect(page.getByText(/file\(s\) imported successfully/i)).toBeVisible({ timeout: 10000 })

    // Verify the success message mentions EquatePlus
    await expect(page.getByText(/from EquatePlus/i)).toBeVisible()

    // Verify transactions table is visible
    await expect(page.getByRole('heading', { name: 'Transactions', exact: true })).toBeVisible()

    // Check for some transaction data from the EquatePlus example
    await expect(page.getByText('BP').first()).toBeVisible() // BP ticker
    await expect(page.getByText('EquatePlus').first()).toBeVisible()
  })
})
